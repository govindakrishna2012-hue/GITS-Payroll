...
const RAW_URL = "https://vmntpwethpuvptczrfft.supabase.co/rest/v1/";
const RAW_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZtbnRwd2V0aHB1dnB0Y3pyZmZ0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY0MzQ3ODUsImV4cCI6MjA5MjAxMDc4NX0.eAtuPwCE2WH5ReV1cXaxah6c4hxdo2pjS8d62nWIKCo";

// 🛡️ BULLETPROOF AUTO-CLEANER 🛡️
const SUPABASE_URL = RAW_URL.replace(/\/rest\/v1\/?$/i, '').replace(/\/$/, '').trim();
...
const LOGO_PATH = "/logo.png"; // Clean path for your uploaded logo
    ...
    <div style={{display:"flex",justifyContent:"center",paddingTop:100,fontFamily:"sans-serif"}}>
      <div style={{width:320,padding:30,border:"1px solid #ddd",borderRadius:12,boxShadow:"0 4px 12px rgba(0,0,0,0.1)"}}>
        ...
        <div style={{textAlign:"center", marginBottom:20}}>
           <img src={LOGO_PATH} alt={CO} style={{maxHeight:70, maxWidth:'100%'}} onError={(e)=>{e.target.style.display='none'; e.target.nextSibling.style.display='block';}} />
           <h2 style={{margin:0, display:'none'}}>{CO}</h2>
        </div>
    ...
    <div style={{padding:20,fontFamily:"sans-serif",maxWidth:1200,margin:"auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"2px solid #1a1a2e",paddingBottom:10,marginBottom:20}}>
        ...
        <div style={{display:"flex", alignItems:"center", gap: 15}}>
          <img src={LOGO_PATH} alt={CO} style={{maxHeight: 50}} onError={(e)=>{e.target.style.display='none'; e.target.nextSibling.style.display='block';}} />
          <h2 style={{margin:0, display:'none'}}>{CO}</h2>
          <div>
            <h3 style={{margin:0, color: "#1a1a2e"}}>{ses.role==="e" ? "Employee Portal" : "Admin Dashboard"}</h3>
            <small>{AD}</small>
          </div>
        </div>
