...
// --- DATABASE CONNECTION (RESTORED KEY) ---
...
export default function App() {
  ...
  if (!db.loaded) return <div style={{padding:50, textAlign:'center'}}><h2>GATEWAY IT SOLUTIONS</h2><p>Restoring Secure Connection...</p></div>;
    ...
    <div style={{ padding: 20, fontFamily: 'sans-serif', maxWidth: 1100, margin: 'auto' }}>
      ...
      {tab === "attendance" && (
        <div style={{marginTop:30}}>
            <div style={{marginBottom:20}}>
                <button onClick={() => setSub("daily")} style={{background: sub==="daily"?'#1a1a2e':'#eee', color: sub==="daily"?'#fff':'#000', padding:10, marginRight:10, border:'none'}}>1. Daily Entry</button>
                <button onClick={() => setSub("report")} style={{background: sub==="report"?'#1a1a2e':'#eee', color: sub==="report"?'#fff':'#000', padding:10, border:'none'}}>2. FY 2026-27 Report</button>
            </div>

            {sub === "daily" ? (
                <div style={{padding:20, border:'1px solid #ddd', borderRadius:8}}>
                    <h4>Mark Attendance for {selDate}</h4>
                ...
                <div style={{padding:20, border:'1px solid #ddd', borderRadius:8}}>
                    ...
                    <table style={{width:'100%', borderCollapse:'collapse', marginTop:15}}>
                        <thead><tr style={{background:'#f4f4f4'}}><th>Name</th><th>Presents</th><th>Leave Used</th><th>LOP Days</th></tr></thead>
                        <tbody>
                            {db.emps?.map(e => {
                                // STRICT YEAR FILTER
      ...
      {tab === "dashboard" && (
          <div style={{marginTop:50, textAlign:'center'}}>
              <h1>GATEWAY PORTAL V2 ONLINE</h1>
              <p>Database Connected. All 2025-26 data is safely archived in the cloud.</p>
          </div>
      )}
    </div>
  );
