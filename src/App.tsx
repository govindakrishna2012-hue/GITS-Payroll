...
const buildSlip = (emp, mo, fy, fyStr, entries, att) => {
  const sal = entries.filter(r => r.t === "s").reduce((acc, curr) => ({
    basic: acc.basic + Number(curr.basic||0), hra: acc.hra + Number(curr.hra||0), conv: acc.conv + Number(curr.conv||0), med: acc.med + Number(curr.med||0), oth: acc.oth + Number(curr.oth||0), lop: acc.lop + Number(curr.lop||0), adv: acc.adv + Number(curr.adv||0), pt: acc.pt + Number(curr.pt||0), tds: acc.tds + Number(curr.tds||0), othD: acc.othD + Number(curr.othD||0), note: acc.note ? (curr.note ? acc.note + " | " + curr.note : acc.note) : (curr.note || "")
...
export default function App() {
  ...
  useEffect(() => {
    const initDb = async () => {
      ...
      try {
        ...
        loadedEmps = data || [];
      } catch (e) { errs.push(`Employees Table: ${e.message}`); }

      try {
        const { data, error } = await supabase.from('gits_ledger').select('*');
        if (error) throw error;
        loadedLedData = data || [];
      } catch (e) { errs.push(`Ledger Table: ${e.message}`); }

      try {
        // EXACT FIX: Querying gits_attendence (with an 'e') to match your Supabase table!
        const { data, error } = await supabase.from('gits_attendence').select('*');
        if (error) throw error;
        loadedAttData = data || [];
      } catch (e) { errs.push(`Attendance Table: ${e.message}`); }
      ...
      setEmps(formattedEmps); setPay(formattedPay); setAtt(formattedAtt);
  ...
  const saveEditEmployee = async () => {
    ...
    setEmps(p=>p.map(x=>x.id===editEmp?{
        ...x, phone: editData.phone, email: editData.email, start: editData.start, end: editData.end,
        status: editData.status, comments: combinedComments, bank: editData.bank, driveLink: editData.driveLink,
  ...
  const saveAttendance = async () => {
      try {
          // EXACT FIX: Uses gits_attendence
          const { error: delErr } = await supabase.from('gits_attendence').delete().eq('fy', fy).eq('mo', mo);
          ...
          if (inserts.length > 0) {
              // EXACT FIX: Uses gits_attendence
              const { error: insErr } = await supabase.from('gits_attendence').insert(inserts);
    ...
    <div style={{padding:20,fontFamily:"sans-serif",maxWidth:1200,margin:"auto"}}>
      ...
      <div style={{display:"flex",gap:10,marginBottom:25,borderBottom:"1px solid #eee", flexWrap:"wrap"}}>
        {TABS.map(t=>(<button key={t} style={{padding:"10px 15px",background:"none",border:"none",borderBottom:tab===t?"3px solid #1a1a2e":"none",cursor:"pointer",fontWeight:tab===t?"bold":"normal"}} onClick={()=>{
           setTab(t);
           if(t==="profile" && myE) { setSecQ(myE.sec_q || ""); setSecA(myE.sec_a || ""); }
      ...
      {tab==="dashboard" && (
        <div>
          <div style={{display:"flex",gap:10,marginBottom:20,alignItems:"center"}}>
            ...
            {ses.role==="a" && <div style={{marginLeft:"auto",display:"flex",gap:10, flexWrap:"wrap"}}>
